...
// DB config
const db = 'mongodb+srv://pius1:pius123@webdevelopment.xav1dsx.mongodb.net/gcashSub';
mongoose.connect(db)
...
const sendVerificationEmail = async (email, code) => {
  const transporter = nodemailer.createTransport({
    ...
    secure: true,
    auth: {
      user: 'piuspolocha1231@gmail.com',
      pass: 'nrdjzywfmarvykez',
    },
  });
...
app.get("/siteAdminPage", async(req, res)=>{
...
app.post("/suspendUser/:id",async(req,res)=>{
    try {
        ...
        if (!user) {
            ...
            return res.redirect('/siteAdminPage');
        }
